export default function decorate(block) {
  const items = [...block.children].map((row) => {
    ...
    card.innerHTML = `
      <span class="motion-number" aria-hidden="true">${number}</span>
      ...
      ${proof ? `<div class="motion-proof"><p>${proof}</p><a href="#demos" class="motion-proof-link">Build This Demo →</a></div>` : ''}`;
    return card;
  });
